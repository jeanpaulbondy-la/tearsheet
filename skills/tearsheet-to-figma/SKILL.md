...
Run it, then create each step of each ramp as a Figma primitive variable named `primitive/<short-descriptive-color-name>/<step>` (e.g. `primitive/rust/500`, `primitive/rust/100`) — pick the descriptive name yourself from what you see (a hue/mood word, not "color1"). The 500 step is always the original extracted hex.
...
- **Page styling — apply to every page you create, no exceptions:** right after creating or switching to a page, set `figma.currentPage.backgrounds` to the semantic bg token from 4a (`color/bg/primary` for most pages; `color/bg/surface` is fine for a page that wants to read as a raised panel), bound via `boundVariables` — never leave a page on Figma's default white. Do this for Cover, References, Foundations, Components, and any other page you add, not just the ones that visually seem to need a dark backdrop. Any text placed directly on a page — titles, section headers, References captions — must use one of the text styles from 4b, never Figma's default Inter/Regular that new text nodes start with.
